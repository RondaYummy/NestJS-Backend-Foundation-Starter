...
> **Додано без зміни старих пунктів:** review baseline `(53)` додав `P2-16`, `P2-17`, `P2-18`, `P3-05` та verification checks `V-22`–`V-25`.
...
| `P1-08` | Confirmed defect                            | Виправити невалідний static export `TOKENS.OutboxProcessorOptions` у `OutboxProcessorModule`, який ламає Worker bootstrap |
...
| `P2-16` | Confirmed defect       | Уніфікувати Redis startup config з Nest typed configuration                    |
| `P2-17` | Architectural risk     | Додати provider overrides до `AuthModule.forRootAsync()` для auth stores       |
| `P2-18` | Confirmed defect       | Виправити Jest config imports, які ламають root typecheck і test scripts       |
...
| `P3-05` | Documentation mismatch  | Узгодити Docker/local env приклади для PostgreSQL і Redis                             |
...
## P1-08. Виправити невалідний static export `TOKENS.OutboxProcessorOptions` у `OutboxProcessorModule`
...
- symbol: `@Module({ ... })` метадані класу `OutboxProcessorModule`
...
- `TOKENS.OutboxProcessorOptions` **не** реєструється у власних `providers` цього модуля. Його надає імпортований `OutboxProcessorOptionsModule`, який додається лише динамічно в `forRoot()` / `forRootAsync()`.
- `WorkerModule` (`apps/worker/src/worker.module.ts`) — єдиний entrypoint, що імпортує `OutboxProcessorModule.forRootAsync(...)`. Під час bootstrap Worker падає:

```text
UnknownExportException [Error]: Nest cannot export a provider/module that is not a part of
the currently processed module (OutboxProcessorModule). Please verify whether the exported
Symbol(OutboxProcessorOptions) is available in this particular context.
    at Module.validateExportedProvider (@nestjs/core/injector/module.js)
    at DependenciesScanner.reflectExports (@nestjs/core/scanner.js)
```

- `@nestjs/core` `11.1.27`, `validateExportedProvider` дозволяє експортувати лише токен із власних `providers` модуля або **клас модуля** з його `imports`. Символьний токен, наданий імпортованим модулем, під жодну з умов не підпадає.

### Що зараз не так

Статичні метадані декоратора `@Module` завжди обробляються при скануванні і зливаються з динамічними метаданими `forRoot*`. Тому `exports: [..., TOKENS.OutboxProcessorOptions]` валідовується у scope самого `OutboxProcessorModule`, де цей токен не є ні власним provider, ні класом модуля в `imports` — це невалідний реекспорт.

### Чому це проблема

Worker entrypoint взагалі не стартує (повний outage фонової обробки Outbox/email), хоча API продовжує компілюватися й працювати. Дефект latent з коміту `P2-13`: для Worker немає `worker.module.spec.ts`, а `tsc`/build його не ловить, тому він виявився лише при реальному bootstrap у Docker.
...
WorkerModule
  -> OutboxProcessorModule.forRootAsync(...)
       imports: [ OutboxProcessorOptionsModule.forRootAsync(...) , AuditModule, EventsModule, LoggerModule ]
       @Module.exports (static): [ TOKENS.OutboxProcessor, TOKENS.OutboxProcessorOptions ]
                                                              ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
         provided by imported OutboxProcessorOptionsModule, not by own providers
         -> validateExportedProvider -> UnknownExportException -> Worker bootstrap fails
```

Споживач токена на рівні Worker: `outboxProcessorProvider` (`apps/worker/src/processors/outbox-processor.provider.ts`) інжектить `TOKENS.OutboxProcessorOptions` і `TOKENS.OutboxProcessor`, тому обидва мають лишатися видимими для `WorkerModule`.

### Що потрібно змінити

1. Прибрати `TOKENS.OutboxProcessorOptions` зі статичного `exports` декоратора `@Module` (залишити `TOKENS.OutboxProcessor`).
2. У `forRoot()` / `forRootAsync()` захопити dynamic module опцій в один instance і реекспортувати сам модуль: додати його і в `imports`, і в `exports` об'єкта, що повертається.
3. Реекспорт модуля опцій робить `TOKENS.OutboxProcessorOptions` видимим для `WorkerModule` (`outboxProcessorProvider`), зберігаючи наявний публічний контракт.
4. Додати bootstrap/DI regression-тест для Worker (або цільовий тест `OutboxProcessorModule.forRootAsync(...)`), який компілює модуль і резолвить обидва токени, щоб дефект не повернувся.

### Acceptance criteria

- `OutboxProcessorModule.forRootAsync(...)` компілюється без `UnknownExportException`;
- Worker bootstrap проходить стадію сканування модулів; `TOKENS.OutboxProcessor` і `TOKENS.OutboxProcessorOptions` резолвляться у `WorkerModule`;
- Cron і API поведінка не змінюється (Cron і далі використовує `OutboxProcessorOptionsModule` напряму);
- додано regression-тест, що падав би на старому коді;
- `npm run build`, `npm run build:worker`, `npm run lint` і релевантні unit-тести проходять.
...
## P2-16. Уніфікувати Redis startup config з Nest typed configuration
...
- `apps/worker/src/main.ts` і `apps/cron/src/main.ts` мають такий самий pre-Nest Redis startup check.
- `libs/infrastructure/src/redis/redis-startup-config.ts` читає `process.env` напряму:
...
- Реальна application config вантажиться пізніше через `InfrastructureConfigModule` / `ConfigModule.forRoot(...)` і `AppConfigService`.

### Що зараз не так

Redis startup preflight має окремий configuration path до Nest bootstrap. Він читає raw `process.env`, має власні defaults (`REDIS_HOST=localhost`) і може перевіряти інші значення, ніж ті, які пізніше використають `RedisModule.forRootAsync(...)`, `HealthModule` або інші infrastructure modules.

### Чому це проблема

API, Worker або Cron можуть завершитися до Nest bootstrap через Redis preflight, навіть якщо typed Nest configuration була б валідною. Це також дублює env parsing logic і порушує вимогу starter kit мати один явний typed configuration contract.
...
apps/api|worker|cron main.ts
  -> assertRedisAvailable(getRedisStartupConfig())
    -> redis-startup-config.ts
      -> raw process.env + local defaults
...
1. Винести bootstrap env parsing у shared typed loader, який використовується і preflight checks, і `InfrastructureConfigModule`.
2. Або перенести Redis preflight після створення Nest application context і брати options з `AppConfigService`.
3. Не дублювати defaults для Redis у двох різних місцях.
4. Додати component name до startup logging, щоб API/Worker/Cron мали коректний context.
5. Оновити `.env.example` і README, якщо canonical Redis startup env keys зміняться.

### Acceptance criteria

- Redis startup check і `RedisModule.forRootAsync(...)` використовують один і той самий parsed config;
- invalid Redis env fail-fast-иться однаково в API, Worker і Cron;
- немає прихованого fallback на `localhost`, якщо configured runtime очікує Docker service name `redis`;
- API/Worker/Cron startup logs показують правильний component context.

---

## P2-17. Додати provider overrides до `AuthModule.forRootAsync()` для auth stores
...
- `libs/infrastructure/src/auth/auth.module.ts`
- `AuthModule.forRoot(...)` має `AuthModuleRegistrationOptions` з `imports` і `providers`.
- `AuthModule.forRootAsync(...)` приймає лише:
...
- `buildAsyncDriverProviders()` жорстко створює Redis-backed stores:
...
Sync registration уже має механізм custom store providers, але async registration — основний production сценарій з `AppConfigService` — такого extension point не має. Для заміни JWT/session store у `forRootAsync()` потрібно редагувати internals `AuthModule`.

### Чому це проблема

Starter kit заявляє незалежну переносимість модулів і можливість заміни adapters без зміни внутрішнього коду. Поточний async Auth contract приховано прив’язує модуль до `RedisService` та Redis-backed stores.
...
AuthApplicationCompositionModule.register(...)
  -> AuthModule.forRootAsync(...)
    -> buildAsyncDriverProviders()
      -> inject RedisService
      -> new RedisJwtTokenStore(redis) або new RedisSessionStore(redis)
      -> TOKENS.AuthTokenService
```

### Що потрібно змінити

1. Розширити `AuthModuleAsyncOptions` полем `providers?: Provider[]` або окремим typed `storeProviders` contract.
2. Дозволити consumer-у передавати `TOKENS.JwtTokenStore` / `TOKENS.SessionStore` у async registration.
3. У `buildAsyncDriverProviders(...)` не створювати Redis-backed store, якщо передано custom provider.
4. Залишити Redis як default adapter, але зробити його явним default, а не hardcoded єдиним варіантом.
5. Оновити `EXAMPLES.md` з прикладом custom auth store для `forRootAsync()`.

### Acceptance criteria

- `AuthModule.forRootAsync()` підтримує custom JWT/session store provider без редагування `auth.module.ts`;
- default Redis-backed behavior зберігається;
- consumer module явно імпортує Redis тільки коли використовує Redis store;
- targeted DI test підтверджує async auth registration з custom store без `RedisModule`.

---

## P2-18. Виправити Jest config imports, які ламають root typecheck і test scripts
...
jest.module.config.ts(3,24): error TS5097: An import path can only end with a '.ts' extension when 'allowImportingTsExtensions' is enabled.
jest.release.config.ts(3,24): error TS5097: An import path can only end with a '.ts' extension when 'allowImportingTsExtensions' is enabled.
jest.unit.config.ts(3,24): error TS5097: An import path can only end with a '.ts' extension when 'allowImportingTsExtensions' is enabled.
```

Команда:

```text
npm run test:module -- --silent
```

Результат:

```text
Jest: Failed to parse the TypeScript config file .../jest.module.config.ts
...
- `jest.unit.config.ts`;
- `jest.module.config.ts`;
- `jest.integration.config.ts`;
- `jest.release.config.ts`.
...
Root TypeScript check і Jest scripts падають ще на parsing config, до запуску тестів. Це окремий дефект від `P3-04`, де йдеться про timeout/open handles після старту test suite.

### Чому це проблема

Агенти й CI не мають робочого verification gate для module/release/unit тестів. Неможливо відрізнити реальні test failures від поламаного tooling bootstrap.

### Що потрібно змінити

1. У всіх `jest.*.config.ts` замінити import на extensionless path:

```ts
import baseConfig from './jest.config.base';
```

2. Не вмикати `allowImportingTsExtensions` глобально лише для обходу цієї локальної проблеми, якщо це не є усвідомленим project-wide рішенням.
3. Повторити root typecheck і всі Jest commands.
4. Після цього окремо повернутися до `P3-04`, якщо test suite все ще має timeout/open handles.

### Acceptance criteria

- `npx tsc --noEmit -p tsconfig.json --pretty false` не падає на `TS5097`;
- `npm run test:module -- --silent` доходить до виконання suites;
- `npm run test:unit`, `npm run test:int`, `npm run test:release` не падають на parsing config;
- `P3-04` залишається окремим пунктом тільки для runtime test stability.
...
## P3-05. Узгодити Docker/local env приклади для PostgreSQL і Redis
...
README показує різні варіанти `DATABASE_URL`:
...
і в quickstart пропонує flow на кшталт:
...
- `docker-compose.yml` використовує `POSTGRES_USER=postgres`, `POSTGRES_PASSWORD=postgres`.
- Якщо `npm run start:dev:api` запускається з host machine, hostname `postgres` / `redis` із `.env.example` зазвичай не резолвиться поза Docker network.

### Що зараз не так

Один documented local flow змішує Docker-internal hostnames (`postgres`, `redis`) із host-run Node process. Також README місцями використовує `app:app`, тоді як compose і `.env.example` використовують `postgres:postgres`.

### Чому це проблема

Новий інтегратор може отримати connection failure не через код, а через суперечливий quickstart. Це збільшує час onboarding і створює помилкові runtime reports.

### Правильна цільова поведінка

Має бути два явні env профілі або один несуперечливий flow:

```text
.env.example.local   -> localhost ports для host-run npm scripts
.env.example.docker  -> postgres/redis service names для container-run
```

або README має чітко пояснювати, які значення змінити після `cp .env.example .env`.

### Потрібно змінити

1. Узгодити credentials у `README.md`, `.env.example` і `docker-compose.yml`.
2. Розділити local та Docker env приклади.
3. У quickstart явно вказати:
   - для host-run: `DATABASE_URL=...@localhost:5432`, `REDIS_HOST=localhost`;
   - для container-run: `DATABASE_URL=...@postgres:5432`, `REDIS_HOST=redis`.
4. Перевірити `P2-16`, щоб Redis startup check не fallback-ився на інший hostname.

### Acceptance criteria

- README має один canonical Docker credentials приклад;
- `.env.example` не вводить в оману для host-run `npm run start:dev:*`;
- Docker compose і docs використовують однакові DB user/password/database;
- local quickstart проходить із documented hostnames без ручного reverse engineering.
...
| Docker/local env examples змішують service names, localhost і різні DB credentials     | `P3-05`          |
...
| `V-14` | Unit suite timeout/root cause and ts-jest configuration — see command matrix below |
...
| Full test gate    | `npm run test:all`                           | Exit 0 (unit + module + release; no infra)     |
| Infra integration | `npm run test:int`                           | Exit 0 when PostgreSQL and Redis are available |
