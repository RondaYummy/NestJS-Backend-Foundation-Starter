...
Успіх — `200` з login-подібною відповіддю (`data.user` + `data.auth`), а клієнт одразу отримує свіжі креденшли. У режимі `AUTH_DRIVER=session` сервер також встановлює новий cookie `sid`.

Що відбувається з попередніми креденшлами:

- `authVersion` збільшується — усі старі JWT/session креденшли не проходять перевірку свіжості;
- перед видачею нових артефактів сервер **еагерно** чистить Redis: усі проіндексовані сесії користувача (`AUTH_DRIVER=session`) або всі проіндексовані refresh-token families (`AUTH_DRIVER=jwt`);
- уже видані **access** JWT не потрапляють у blacklist: вони відхиляються на наступному запиті лише тоді, коли composition root підключив `resolveAccessUser` (за замовчуванням у starter kit). Портативні збірки без `resolveAccessUser` прийматимуть їх до закінчення TTL;
- families, видані до апгрейду на цю поведінку, не мають запису в per-user індексі — вони не чистяться еагерно, але залишаються нечинними через `authVersion` і зникають за TTL.
...
Успіх — `200` з login-подібною відповіддю та свіжими auth-артефактами (як change-password), включно з тим самим еагерним очищенням Redis-сесій / refresh-families і bump `authVersion`. Токен одноразовий: повторне використання, протермінований або невідомий токен повертає `400 INVALID_RESET_TOKEN` без зміни пароля.
